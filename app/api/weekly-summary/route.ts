...
export async function GET(request: Request) {
  ...
  const buildSections = (dueList: typeof allDue, overdueList: typeof allOverdue) => {
    ...
    return `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111;">
      ...
      <p style="color:#666;margin-top:0;">${customIntro || `Settimana del ${weekLabel}`}</p>
      ${dueSection}${overdueSection}
    </div>`;
  };

  const errors: string[] = [];
  const subjectBase = cfg.subject || "📅 Riepilogo settimanale";
  ...
  if (dueTiz.length > 0 || overdueTiz.length > 0) {
    try {
      await brevo.transactionalEmails.sendTransacEmail({
        ...
        subject: `${subjectBase} — ${dueTiz.length} in scadenza, ${overdueTiz.length} scaduti`,
  ...
  if (dueChiara.length > 0 || overdueChiara.length > 0) {
    try {
      await brevo.transactionalEmails.sendTransacEmail({
        ...
        subject: `${subjectBase} — ${dueChiara.length} in scadenza, ${overdueChiara.length} scaduti`,
  ...
  return NextResponse.json({ ok: true, sent: true, dueThisWeek: allDue.length, overdue: allOverdue.length });
}
